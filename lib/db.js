...
module.exports = function(Config) {
  ...
  module.getLocations = function(queryStr, callback) {
    ...
    var query = "SELECT l.*, st_y(l.the_geom) as lat, st_x(l.the_geom) as long FROM locations l LEFT JOIN locations_offerings lo on l.cartodb_id = lo.location_id LEFT JOIN offerings o on" 
  ...
  module.getLocationsByAddress = function(latlng, callback) {
    var query = "SELECT * FROM locations WHERE ST_Distance_Sphere(the_geom, ST_GeomFromText('{{geom_text}}')) < 25" 
    var geom_text = 'POINT(' + latlng[1] + ' ' + latlng[0] + ')';
    var options = {'geom_text': geom_text};
    this._query(query, options, callback);
  };
